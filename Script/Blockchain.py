...
def check_3():                                            # Проверка правильная ли последовательность хэшей у блоков
    for z in range(1000000000):                                # Проверка блоков на правильность
        try:
            file_z = open('Database/Blockchain_processing_1/' + str(z) + '.txt', 'r')             # открытие блока под номером i
            file_z1 = open('Database/Blockchain_processing_1/' + str(z+1) + '.txt', 'r')          # открытие блока под номером i + 1
            ...
            shutil.copyfile('Database/Blockchain_processing_2/' + str(z) + '.txt', 'Blockchain/' + str(z) + '.txt')
            print('Блок номер', z, 'Добавлен в блокчейн')

            if block_z[6][11:] == block_z1[4][15:]:  # Цикл для проверки предыдущих хэшей блока
                shutil.copyfile('Database/Blockchain_processing_2/' + str(z+1) + '.txt', 'Blockchain/' + str(z+1) + '.txt')
                print('Блок номер', z, 'Добавлен в блокчейн')
            else:
...
def client_clean():
    ...
    for z in range(1000000000):
        try:
            print(z)
            os.remove('Database/Blockchain_processing/' + str(z) + '.txt')

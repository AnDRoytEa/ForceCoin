...
def create_block_if_nothing_to_mine():                           # Создание блока для майнинга если других блоков для майнинга не существует
    ...
    f = open('System/Database/users.txt', 'r')               # Количество существующих аккайнтов
    number_of_users = len(f.readlines())
    f.close()
    f = open('System/Database/users.txt', 'r')               # Нахождение случайного пользователя
    users = f.readlines()
    f.close()
    #for i in range(14):
    ...
    if len(os.listdir('System/Block_that_needs_mining/')) == 0:
        for z in range(maxx_block, maxx_block + 1): # Создание 2 блоков если нечего майнить
            ...
            f.writelines('sent_coin: ' + '0.000001' + '\n')
